...
;joenote - So what we've just done here is effectively halved the available exp pool ( = X / 2)
;The amount of exp being dealt out is dependent on the values in the wEnemyMonBaseStats structure
...
;joenote - as intended, half the exp pool is divided again by the number "N" of non-fainted pokemon that fought
;The exp value that is currently stored = X / [2N]
;N's value can be found in the function DivideExpDataByNumMonsGainingExp
...
;joenote - Uh oh! We're about to distribute the stored exp values among all non-fainted party members "P"
;But the exp distributed to each one = X / [2NP] when it is supposed to be = X / [2P]
;The solution would be to store "N" earlier, then at this point multiply the current exp values by N
;The make sure to store them back into the wEnemyMonBaseStats structure in the proper locations
...
.next4
	...
	ld de, wEnemyMonDVs;shiny code
	callfar IsMonShiny
	jr z, .noFlash
	ld hl, wShinyMonFlag
	set 1, [hl]
	callfar PlayShinySparkleAnimation
.noFlash;shiny code end	
	ld a, [wEnemyMonSpecies2]
	call PlayCry
...
DrawPlayerHUDAndHPBar:
	...
	call CenterMonName
	call PlaceString
	farcall PrintEXPBar;new code
	call PrintPlayerMonShiny;shiny code
...
DrawEnemyHUDAndHPBar:
	...
	call CenterMonName
	call PlaceString
	call PrintEnemyMonShiny;shiny code
	hlcoord 4, 1
	push hl
	inc hl
	ld de, wEnemyMonStatus
...
PrintPlayerMonShiny:;shiny code
	ld de, wBattleMonDVs
	hlcoord 10, 8
	jr PrintShinyCommon
...
.focusEnergyUsed
	;srl b
	sla b ;Fixed Focus Energy effectivness
.noFocusEnergyUsed
