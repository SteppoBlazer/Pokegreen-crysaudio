...
SetPal_Overworld:
	...
	ld bc, $10
	call CopyData
; Daynght new palette switch method
    call CheckDayNight
    ld hl, PaletteMappings
    jr c, .dayTime 
    ld hl, PaletteMappingsNight 
.dayTime
    ld a, [wCurMap] ; id of the current map
    ld d, 0
    ld e, a ; these two lines are making de = a, we have to do this to be able to add a to hl easily
    add hl, de ; navigate to the address containing the current map's palette ID in the list by adding the current map ID to the address of the start of the list
    ld a, [hl] ; get the palette ID for the given map stored in the current address hl is at
	; a is the palette ID!
; Daynght new palette switch method ends
	ld hl, wPalPacket + 1
	ld [hld], a
	ld de, BlkPacket_WholeScreen
...
.skipDexNumConversion
	...
	add hl, de
	ld [wd11e], a;shiny code
	and a ; is the mon index 0? shiny code end
	ld a, [hl]	
	ret z;shiny code
	push bc
	ld d, a
...
INCLUDE "data/sgb/palette_mappings.asm"

INCLUDE "data/sgb/palette_mappings_night.asm"
